...
PUBLIC MST_API void MST_add_data_to_array(MST_Object *mst_obj, Array *arr,
                                          Array_data ad);
PUBLIC MST_API char *MST_build_to_string(MST_Object *mst_obj);
PUBLIC MST_API void MST_add_var_to_space(MST_Object *mst_obj, SPACE *sp,
                                         Var var);
...
PUBLIC MST_API void MST_change_arr(MST_Object *mst_obj, Array_data v,
                                   Array_data *v1);
...
#define MST_get_string_in_space(m, name, space)                                \
  MST_get_var((name), (space))                                                 \
      ? MST_space_get_str(MST_get_var((name), (space)))                        \
      : NULL // MST_get_string_in_space
#define MST_get_string_in_array(m, idx, arr)                                   \
  MST_array_get_data((arr), (idx))                                             \
      ? MST_array_get_str(MST_array_get_data((arr), (idx)))                    \
